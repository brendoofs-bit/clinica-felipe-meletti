  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-40 transition-all duration-500 ${
        scrolled 
          ? 'bg-dark-900/70 backdrop-blur-xl shadow-lg border-b border-white/10 py-4' 
          : 'bg-transparent py-6'
      }`}
    >
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex justify-between items-center">
        ...
        <div className="flex flex-col items-end">
           <span className={`text-[0.6rem] md:text-xs uppercase tracking-[0.3em] font-bold mb-[-4px] z-10 transition-colors duration-300 ${scrolled ? 'text-gold-400' : 'text-gold-400'}`}>
             Clínica
           </span>
           <h1 className={`font-serif text-2xl md:text-3xl tracking-wide font-bold transition-colors duration-300 ${scrolled ? 'text-gold-500' : 'text-gold-500'}`}>
             DR. FELIPE MELETTI
           </h1>
        </div>
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          <a 
            ...
            className={`px-6 py-2 border rounded-full text-xs uppercase tracking-widest hover:bg-gold-500 hover:border-gold-500 hover:text-white transition-all duration-300 ${
               scrolled ? 'border-gold-500 text-gold-500' : 'border-white text-white'
            }`}
          >
            Agendar
          </a>
